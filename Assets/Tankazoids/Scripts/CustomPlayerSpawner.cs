...
namespace FishNet.Component.Spawning
{
    ...
    public class CustomPlayerSpawner : MonoBehaviour
    {
        ...
        private void SceneManager_OnClientLoadedStartScenes(NetworkConnection conn, bool asServer)
        {
            ...
            print(nob);
            _mapManager.addPlayer(conn, nob.GetComponent<Tank>()); //is this nonsense
        ...
        public void Respawn(NetworkConnection conn, Vector3 position) //asServer bool? umm
        {
            Vector3 dummyPosition;
            Quaternion rotation;
            SetSpawn(_playerPrefab.transform, out dummyPosition, out rotation);
            NetworkObject nob = Instantiate(_playerPrefab, position, rotation);
            _networkManager.ServerManager.Spawn(nob, conn);
            _mapManager.addPlayer(conn, nob.GetComponent<Tank>()); //is this nonsense
